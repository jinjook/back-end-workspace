/*
    ...
    - 가상 테이블 (실제 데이터가 담겨 있는 건 아님 -> 논리적인 테이블)
...
/*
    ...
    - OR REPLACE : 뷰 생성 시 기존에 중복된 이름의 뷰가 없다면 새로 뷰 생성, 있다면 해당 뷰 수정
*/
...
AS SELECT emp_id 사번, emp_name 사원명, job_name 직급명, if(substr(emp_no, 8, 1) = 2, '여','남') 성별, timestampdiff(year, hire_date, current_time) 근무년수
	FROM employee
...
-- 뷰를 통해서 INSERT / job 테이블에도 추가 됨
INSERT INTO vw_job
VALUES ('J8', '인턴'); 

-- 뷰를 통해서 UPDATE / job 테이블에도 수정 됨
UPDATE vw_job
SET job_name = '알바'
WHERE job_code = 'J8';

-- 뷰를 통해서 DELETE / job 테이블에도 삭제 됨
DELETE FROM vw_job
WHERE job_code = 'J8';
...
VALUES('J8', '인턴'); -- job_name이 없음

INSERT INTO vw_job
VALUES('J8'); -- job 테이블에 job_name은 null 로 저장됨
...
INSERT INTO vw_job
VALUES('인턴'); -- 자체 쿼리문에는 문제가 없으나, job_code에 primary key가 걸려있기 때문에 not null 불가!
...
DELETE FROM vw_job
WHERE job_name = '인턴'; -- foreign key로 연결되어있는 값이 있어 삭제 불가 / 자식 데이터가 존재하지 않는 경우만 삭제 가능
...
VALUES(300, '홍길동', 3000000, 36000000); -- 연봉에서 에러 / 산술연산식으로 정의된 컬럼은 데이터 추가 불가능

-- 산술연산식이 컬럼에 같이 들어간 경우도 데이터 추가 불가능
INSERT INTO vw_emp_sal(사번, 사원명, 주민번호, 급여) 
VALUES(301, '홍길동', '910316-1965216', 3000000);
    
-- 산술연산으로 정의된 컬럼은 데이터 변경 불가능!
UPDATE vw_emp_sal
SET 연봉 = 8000000
WHERE emp_id = 200; -- 불가능

UPDATE vw_emp_sal
SET 급여 = 9000000
WHERE 사번 = 200; -- 가능 / 산술연산과 무관한 컬럼은 데이터 변경 가능
...
-- 4) 그룹함수나 GROUP BY 절을 포함한 경우 : !INSERT, UPDATE, DELETE 모두 에러!
...
-- 5) DISTINCT 구문이 포함된 경우 : !INSERT, UPDATE, DELETE 모두 에러!
...
VALUES (500, '송준호', '800130-1111111', '총무부'); -- 에러 // foreign key로 연결된 값은 추가 불가능
...
UPDATE vw_joinemp
SET dept_title = '회계부'
WHERE emp_id = 214; -- 가능 / 같은 부서 모두 변경됨

DELETE FROM vw_joinemp
WHERE emp_id = 214; -- delete는 불가능
...
-- 급여가 300만원 이상인 사원들만 조회한 SELECT문을 vw_emp 뷰 정의
CREATE OR REPLACE VIEW vw_emp
AS SELECT *
	FROM employee
    WHERE salary >= 3000000;
    
-- 200번 사원의 급여를 200만원으로 변경 (vw_emp 이용)
UPDATE vw_emp
SET salary = 2000000
WHERE emp_id = 200;
...
AS SELECT *
	FROM employee
    WHERE salary >= 3000000
WITH CHECK OPTION; -- 수정해서 조건에 부합하지 않게 되면 에러 발생

-- 202번 사원의 급여를 200만원으로 변경 (vw_emp 이용)
UPDATE vw_emp
SET salary = 2000000
WHERE emp_id = 202; -- 불가능 / 수정 후 조건에 부합하지 않아서

-- 202번 사원의 급여를 400만원으로 변경 (vw_emp 이용)
UPDATE vw_emp
SET salary = 4000000
WHERE emp_id = 202; -- 가능 / 수정했을 때 여전히 조건에 부합하기 때문에 
