/*
    ...
    >> SELECT절에 단일행 함수와 그룹 함수는 함께 사용하지 못함 -> 결과 행의 갯수가 다르기 때문
...
/*
    ...
    - 한글 한 글자를 3BYTE로 계산함
    - 영문자, 숫자, 특수문자는 한 글자를 1BYTE로 계산 -> CHAR_LENGTH와 동일
    CHAR_LENGTH : 해당 문자열값의 글자 수 반환
*/
SELECT length('데이터베이스'), -- 18
	char_length('데이터베이스'), -- 6
    length('database'), -- 8
...
/*
    ...
    - 없으면 0 반환
*/
SELECT instr('AABAACAABBAA', 'B'), -- 3 / 여러개 중 가장 처음꺼 위치로 반환됨
	instr('AABAACAABBAA', 'D'); -- 0
...
/*
    ...
    - 문자열에 덧붙이고자 하는 문자를 왼쪽 또는 오른쪽에 덧붙여서 최종적으로 반환할 문자이 길이만큼 문자열 반환
*/
SELECT lpad('hello', 10, '*'), -- *****hello
	rpad('hello', 10, '+'); -- hello+++++
    
/*
	TRIM(컬럼|'문자열') | TRIM([LEADING(왼)|TRAILING(오)|BOTH] 제거하고자하는 문자을 FROM 컬럼|'문자열')
    - 문자열의 앞/뒤/양쪽에 있는 지정한 문자들을 제거한 나머지 문자열 반환
*/
SELECT trim('       K H        '); -- K H / 기본적으로 양쪽에 있는 공백 제거
SELECT trim(BOTH ' ' FROM '       K H        '); -- 위와 동일한 뜻

SELECT trim(LEADING 'Z' FROM 'ZZZKHZZZ'); -- KHZZZ
SELECT ltrim('       K H        '); -- LTRIM : 앞쪽 '공백'만 제거

SELECT trim(TRAILING 'Z' FROM 'ZZZKHZZZ'); -- ZZZKH
SELECT rtrim('       K H        '); -- RTRIM : 뒤쪽 '공백'만 제거
...
SELECT substr('PROGRAMMING', -8, 3); -- GRA (-8 : 뒤에서부터 여덟번째)
...
/*
	LOWER : 전부 소문자로 변경한 문자열 반환
    UPPER : 전부 대문자로 변경한 문자열 반환
*/
SELECT lower('Welcome To MySQL'), -- welcome to mysql
	upper('Welcome To MySQL'); -- WELCOME TO MYSQL

/*
	REPLACE(컬럼|'문자열', '바꾸고 싶은 문자열', '바꿀 문자열')
    - 특정 위치에 특정 문자열로 변경하여 반환
*/
SELECT replace('서울특별시 강남구 역삼동', '강남구', '서초구'); -- 서울특별시 서초구 역삼동

/*
	CONCAT : 문자열을 하나로 합친 후 결과 반환
*/
SELECT concat('가나다라', 'ABCD', '1234'); -- 가나다라ABCD1234 / 문자열 몇개던 상관 없음

-- 실습문제 ---
-- 1. 이메일의 kh.or.kr을 gmail.com으로 변경해서 이름, 변경 전 이메일, 변경 후 이메일 조회
SELECT emp_name, email, replace(email, 'kh.or.kr', 'gmail.com') 변경후
FROM employee;

-- 2. 사원명과 주민등록번호 (000000-0******)으로 조회
-- 1) replace 사용
SELECT emp_name, replace(emp_no, substr(emp_no, -6, 6), '******') 주민번호
FROM employee;
-- 2) rpad 사용
...
FROM employee; 
-- 3) trim 
SELECT emp_name, email, trim(both '@kh.or.kr' from email) -- trailing도 가능 
FROM employee; 
...
*/
SELECT
	10 DIV 3, 10 / 3, -- 3 / 3.3333 : DIV는 정수만 반환됨
    10 MOD 3, 10 % 3, mod(10, 3); -- 1 / 1 / 1
...
SELECT round(123.567), -- 124
	round(123.567, 1), -- 123.6 (소숫점 첫번째까지)
    round(123.567, -1); -- 120
...
/*
	PERIOD_DIFF(날짜, 날짜) : 두 기간의 차이를 숫자로 변환 (앞시간-뒷시간)
    DATEDIFF(날짜, 날짜) : 두 날짜 사이의 일수를 숫자로 변환 (앞시간-뒷시간)
    TIMEDIFF(날짜, 날짜) : 두 시간의 차이를 날짜 형식으로 반환 (앞시간-뒷시간)
    TIMESTAMPDIFF(날짜단위, 날짜, 날짜) : 두 날짜 사이의 기간을 날짜단위에 따라 변환 !(뒷시간-앞시간)!
    
    * 날짜단위 : YEAR(연도), QUARTER(분기), MONTH(월), WEEK(주), DAY(일), HOUR(시간), MINUTE(분), SECOND(초)
*/
SELECT period_diff(202406, 202411), -- -5 / 
	period_diff(202412, 202406); -- 6
SELECT datediff('2024-12-31', now()), -- 남은 날짜 (일수) 계산
...
SELECT now(), 
    ...
    addtime(now(), "01:10:00"), -- +1시간 10분
    subtime(now(), "01:00:00"); -- -1시간
