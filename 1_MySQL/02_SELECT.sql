   ...
   - SQL (Structured Query Language)
    ...
    - SQL 종류
	 ...
	 3. DCL(Data Control Language) : 데이터 제어어
        - DBMS에 대한 보안, 무결성, 복수 등 DBMS를 제어하기 위한 언어
	 ...
	 5. TCL(Transaction Control Language) : 트랜잭션을 제어하는 언어
        ...
        *transaction : 데이터베이스에서 하나의 논리적 기능을 수행하기 위한 작업의 단위로서, 
					  데이터베이스 시스템에서 복구 및 병행 시행 시 처리되는 작업의 논리적 단위이다.
*/

/*
    SELECT 컬럼(정보), [컬럼, ...]
    FROM 테이블명;(;붙여야 종료됨. 반드시 붙일것)
...
*/
-- employee 테이블에서 직원명(emp_name), 직원 연봉(salary * 12)
...
/*
   ...
   - 산술연산을 하면 컬럼명이 지저분해짐 (ex. salary * 12)
	 이때 컬럼명에 별칭을 부여해서 깔끔하게 정리
   - 별칭을 부여할 때 띄어쓰기 혹은 특수문자가 포함될 경우 반드시 큰따옴표(")로 감싸준다.
*/
SELECT 
	emp_name 직원명, 
    salary "직원 월급", -- 띄어쓰기 있을 경우 "" 사용
    salary * 12 연봉
FROM employee;
...
SELECT DISTINCT job_code -- 직급코드의 중복값을 제거
FROM employee;
...
FROM employee
WHERE dept_code != 'D1';
-- WHERE dept_code <> 'D1'; 도 가능

-- 3. 급여가 400만원 이상인 사람들의 사원명, 부서코드, 급여 조회
...
-- 급여가 350만원 이상 600만원 이하를 받는 사람들의 사원명, 사번, 급여 조회
...
/*
    ...
    비교대상컬럼 LIKE '특정패턴'
    - 특정패턴에는 '%', '_'를 '와일드카드'로 사용
    
    '%' : 0글자 이상 
    비교대상컬럼 LIKE '문자%' => 비교대상컬럼이 문자로 "시작"되는것 조회
    비교대상컬럼 LIKE '%문자' => 비교대상컬럼이 문자로 "끝"나는것 조회
    비교대상컬럼 LIKE '%문자%' => 비교대상컬럼값이 문자가 "포함"되는것 조회 (키워드 검색에 사용)
...
-- 와일드카드와 원하는 글자가 동일 ___ _
-- ESCAPE OPTION : 나만의 와일드 카드를 만들어서 사용
 SELECT emp_id, emp_name, email
 FROM employee
 -- WHERE email LIKE '___$_%' ESCAPE '$'; -- $를 와일드카드로 설정, 그 뒤는 문자로 인식
 -- 언더바 두개일때는 '___$_$_%'로 작성
 WHERE email LIKE '___\_%'; -- 이것도 가능
...
-- WHERE NOT email LIKE '___$_%' ESCAPE '$'; 둘 다 가능

/*
	IS NULL / IS NOT NULL
    - 컬럼값에 NULL이 있을 경우 NULL 값 비교에 사용되는 연산자 ( = NULL 안됨)
*/
-- 보너스를 받지 않는 사원(bonus 값이 null)들의 사번, 이름, 급여, 보너스(bonus) 조회
...
/*
    ...
    비교대상컬럼 IN ('값1', '값2', ....) -> 검색필터에 사용
*/
-- 부서코드가 D5, D6, D8인 부서원들의 이름, 부서코드, 급여 조회
SELECT emp_name, dept_code, salary
FROM employee
-- WHERE dept_code = 'D5' OR dept_code = 'D6' OR dept_code = 'D8'; 를 줄이기
...
/*
    ...
    1 FROM 테이블명
    2 WHERE 조건식
    4 ORDER BY 정렬하고자하는 컬럼값 [ASC/DESC];
    
    - ASC : 오름차순 정렬 (생략시 기본값)
    - DESC : 내림차순 정렬
*/
-- 전체 사원의 사원명, 보너스 조회 (조건은 필요없고 정렬만 필요 -> WHERE 생략)
SELECT emp_name, bonus
FROM employee
-- ORDER BY bonus ; -- 보너스 기준 오름차순 정렬 (null이 맨 앞)
ORDER BY 2 DESC; -- 보너스 기준 내림차순 정렬 (null이 맨 뒤)
-- 숫자로도 가능, 숫자는 컬럼의 순서 (2 = bonus)

/*
	LIMIT
    - ORDER BY 절 보다 뒤에 조건을 걸고 싶을 때 사용
    - 출력되는 행 수를 제한하는 MySQL 전용 비표준 구문 (잘라버리기)
    - 데이터 양을 제한하고자 할 때 유용 (페이징 처리에 주로 사용)
*/
...
FROM employee
ORDER BY salary DESC
LIMIT 5, 10; -- 5번째부터 10명

SELECT emp_name, salary
FROM employee
ORDER BY salary DESC
LIMIT 10 OFFSET 5; -- 5번째부터 10명(위와 동일값)

-- 실습문제 ---
-- 1. 직급코드(job_code)가 J7이거나 J2인 사원들 중 급여가 200만원 이상인 사람들의 모든 칼럼 조회
SELECT *
FROM employee
-- WHERE (job_code = 'J7' OR job_code = 'J2') AND salary >= 2000000;
-- AND가 OR 보다 우선순위 -> 괄호로 우선처리 또는 
...
-- 4. 입사일이 '1995-01-01' 이상이고 부서배치를 받은 사원들의 사번, 사원명, 입사일(hire_date), 부서코드 조회 
--    (정렬은 먼저 입사한 기준)
...
-- 5. 급여가 200만원 이상 500만원 이하이고 입사일이 '2001-01-01' 이상이고 보너스를 받지 않은 사원들의
...
-- 6. 보너스 포함 연봉이 NULL이 아니고 이름이 '하'가 포함되어 있는 사원들의 사번, 사원명, 급여, 보너스 포함 연봉(별칭부여) 조회
--    +) 연봉이 높은 순으로 정렬
--    보너스 포함 연봉 (salary + salary * bonus) *12
SELECT emp_id, emp_name, salary, (salary + salary * bonus) *12 연봉
FROM employee
-- WHERE 연봉 IS NOT NULL 은 안됨
WHERE bonus IS NOT NULL 
	AND emp_name LIKE '%하%'
ORDER BY 연봉 DESC;
-- 정렬은 별칭으로 설정 가능 / 별칭으로 정렬하려면 띄어쓰기 없어야함 ONLY 단어
