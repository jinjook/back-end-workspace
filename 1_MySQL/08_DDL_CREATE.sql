...
/*
    ...
    * 자료형
    1. 문자
		- CHAR / **VARCHAR : 고정(CHAR) 및 가변 길이(VARCHAR) 문자, 반드시 크기 지정 / character
...
-- INSERT INTO 테이블명 VALUES(값, 값, ...); -- 순서대로 작성
...
/*
	위처럼 프라이머리 키(mem_no) 및 id 등 동일한 값이 들어가면 안되거나 / null이 들어가면 안되는 경우가 있음
...
-- >> unique 제약조건에 위배되어 insert 실패 (user01 동일) 
...
-- >> 회원번호(mem_no)가 동일해도 insert가 가능한 것이 문제

/*
	PRIMARY KEY (기본키)
	- 테이블에서 각 행들을 식별하기 위해 사용될 컬럼에 부여하는 제약조건 (식별자 역할)
    ex) 회원번호, 학번, 사원번호, 부서코드, 직급코드, 제품번호, 주문번호, 운송장번호, ...
    -PRIMARY KEY 제약조건을 부여하면 그 컬럼에 자동으로 NOT NULL + UNIQUE 조건이 설정 됨
    - 한 테이블 당 오로지 한 개만 설정
    --> 간혹 복합키 설정도 가능
*/
...
-- >> 복합키 방식으로 지정해서 쓰는 건 가급적으로 추천하진 않음
...
INSERT INTO pro_like VALUES(1, 'A', current_date()); -- 1A
INSERT INTO pro_like VALUES(1, 'B', current_date()); -- 1B
INSERT INTO pro_like VALUES(2, 'B', current_date()); -- 2B 로 인식되어 모두 다르게 인식됨/INSERT 가능해짐
...
CREATE TABLE mem_grade(
	grade_code INT PRIMARY KEY AUTO_INCREMENT, -- MySQL에서 사용 가능 / 1부터 자동 추가
    grade_name VARCHAR(30) NOT NULL
);
INSERT INTO mem_grade(grade_name) VALUES('일반회원'); -- grade_name만 추가하고싶을 땐 테이블명에 ()로 추가
...
INSERT INTO member(mem_id, mem_pwd, mem_name, grade_id) 
    ...
    -- >> 유효한 회원등급 번호가 아니어도 INSERT 가능 (grade_code에 4 없음)
...
INSERT INTO member(mem_id, mem_pwd, mem_name, grade_id) 
	VALUES('user03', 'pass03', '우현성', 4);
    -- >> 유효한 회원등급 번호가 아니어서 오류 (grade_code에 4 없음) = 부모키(parent key)를 찾을 수 없다는 오류 발생
INSERT INTO member(mem_id, mem_pwd, mem_name, grade_id) 
...
-- >> 부모 테이블(mem_grade)에서 데이터값을 삭제할 경우 어떤 문제가 발생하는가
...
DELETE FROM mem_grade
WHERE grade_code = 3;
-- >> 자식 테이블(member)에 3 값을 사용하지 않기 때문에 삭제 가능

DELETE FROM member WHERE mem_no = 4;
-- >> 2값을 갖고있는 값을 삭제 후 grade_code 2 삭제 가능

/*
	자식테이블 생성시 외래키 제약조건을 부여할 때 삭제옵션 지정 가능
    - 삭제옵션 : 부모테이블의 데이터 삭제 시 그 데이터를 사용하고 있는 자식테이블의 값을 어떻게 처리할건지 선택하는 옵션
    
    1. ON DELETE RESTRICTED (기본값)
		: 자식데이터로 쓰이는 부모데이터는 삭제가 아예 안되게 설정
...
DELETE FROM mem_grade
WHERE grade_code = 1; -- >> 삭제 / 단 1을 가져다 쓰고 있는 자식 데이터 값이 NULL로 변경됨
...
-- mem_grade 테이블의 grade_code 2 등급 삭제
DELETE FROM mem_grade
WHERE grade_code = 2;
...
AS SELECT * FROM kh.employee; -- AS는 생략가능
...
-- 테이블 구도만 생성하고 데이터 값은 복사하지 않는 방법
CREATE TABLE employee_copy
SELECT * FROM kh.employee
WHERE 1 = 0; -- 맞지 않는 조건을 삽입 / 모든 행에 대해서 매번 false이기 때문에 테이블 구조만 복사
