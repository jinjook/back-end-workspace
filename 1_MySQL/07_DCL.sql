...
-- 사용자 계정 생성
CREATE USER test IDENTIFIED BY 'test'; -- 사용자 계정 & 비밀번호
...
*/
-- 데이터베이스(스키마) 권한 부여
GRANT ALL PRIVILEGES ON kh.* TO test; -- ALL PRIVILEGES ON : kh 테이블 전체 부여

-- 데이터베이스(스키마) 권한 회수
REVOKE ALL PRIVILEGES ON *.* FROM test; -- 문법적으로 오류 뜨긴 하지만 실행에는 상관 x
...
/*
    ...
    --> 데이터베이스와 스키마를 같은 의미로 사용을 하지만, 몇몇 DBMS에서는 다르게 해석
		MySQL에서는 대부분 두 용어를 같이 사용함
*/
-- 스키마 생성
CREATE SCHEMA sample;

-- 스키마 삭제 (IF EXISTS : 있는 경우만 실행 - 삭제 후 실행해도 오류 안뜸)
