...
INSERT INTO publisher(pub_name, phone) VALUES ('길벗', '032-332-0931');
...
CREATE TABLE member(
    ...
    CONSTRAINT mem_status_check CHECK (status IN ('N', 'Y'))
);

SELECT * FROM member;

INSERT INTO member(member_id, member_pwd, member_name, gender, address, phone)
		VALUE ('user01', 'pass01', '가나다', 'M', '서울시 강남구', '010-1111-2222');
INSERT INTO member(member_id, member_pwd, member_name, gender, address, phone)
		VALUE ('user02', 'pass02', '라마바', 'M', '서울시 서초구', '010-3333-4444');
INSERT INTO member(member_id, member_pwd, member_name, gender, address, phone)
		VALUE ('user03', 'pass03', '사아자', 'F', '경기도 광주시', '010-4444-5555');
...
SELECT bk_title, pub_name, rent_date, adddate(rent_date, +7) '반납 예정일'
FROM rent
	JOIN member ON (rent_mem_no = member_no)
    JOIN book ON (rent_book_no = bk_no)
