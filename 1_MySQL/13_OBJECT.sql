/*
	인덱스(INDEX)
    - SQL 명령문의 처리 속도를 향상시키기 위해 행들의 위치 정보를 가지고 있음
    
    * 데이터 검색 방식
    Table Full Scan : 테이블 데이터를 처음부터 끝까지 검색하여 원하는 데이터를 찾는 방식
	Index Scan : 인덱스를 통해 데이터를 찾는 방식 
    
    기본키(PRIMARY KEY)는 테이블의 각 행을 고유하게 식별함
    실제 테이블 데이터가 모두 인덱스 구조로 저장됨
*/ 
-- 테이블에서 인덱스 조회
...
/*
	프로시저(PROCEDURE)
    - SQL문을 저장하여 필요할 때마다 다시 입력할 필요없이 간단하게 호출해서 사용가능하게 하는 코드 블록
    
    DELIMITER // -- 여기부터 시작이라는 뜻
...
DELIMITER // 
CREATE FUNCTION salary_cal(id INT)
RETURNS INT
DETERMINISTIC
BEGIN 
	DECLARE result INT;
...
-- 함수 호출
SELECT salary_cal(200);

-- 방금 만든 salary_cal 함수를 이용해서 연봉이 4000만원 이상인 사원의 사번, 사원명, 급여, 연봉 조회 (정렬 연봉 높은 순)
SELECT emp_id, emp_name, salary, salary_cal(emp_id) 연봉
FROM employee
WHERE salary_cal(emp_id) >= 40000000
ORDER BY 4 desc;

-- 함수 삭제 DELETE 
-- !데이터 삭제 / DROP : 객체 삭제!
-- IF EXISTS : 존재할때만 실행되도록 해줌 
DROP FUNCTION IF EXISTS salary_cal;
...
/*
    ...
    - 입출고에 대한 데이터가 기록(INSERT) 될 때마다 해당 상품에 대한 재고수량을 매번 수정(UPDATE) 해야 하는 경우
...
-- employee 테이블에 새로운 행이 INSERT 될 때마다 '신입사원이 입사했습니다'라는 메세지를 log 테이블에 자동으로 INSERT 되는 트리거 생성
DELIMITER //
...
CREATE TABLE prodetail (
    ...
    CONSTRAINT status_check CHECK (status IN ('입고', '출고'))
);
...
UPDATE product SET stock = stock + 10 WHERE pcode= 1;

-- 3번 상품이 오늘날짜로 5개 출고
INSERT INTO prodetail(pcode, amount, status) VALUES (3,5,'출고');
-- 3번 상품의 재고수량 5 감소
UPDATE product SET stock = stock - 5 WHERE pcode = 3; 
...
/*
    ...
    1. 개념적 모델링
		- 엔티티(Entity) 추출
        - 엔티티 간의 관계 설정
    2. 논리적 모델링 : ERD(Entity Relationship Diagram) 룰
		- 정규화 작업 (1 ~ 5) ... 3까지만 정규화
			--> 너무 쪼개면 join만 많아짐
    3. 물리적 모델링
		- 테이블을 실질적으로 구성
