/*
    ...
    - 쿼리 자체는 직관적
    - 쿼리는 속도를 중요시함, 서브쿼리는 상대적으로 느림
    - 가급적으로 서브쿼리를 사용하지 않아도 되면 안쓰는걸 추천
...
-- 전지연 사원이 속해있는 부서원들의 사번, 직원명, 전화번호, 직급명, 부서명, 입사일 조회 / 단, 전지연 사원은 제외
...
-- SUBQUERY사용 - 훨씬 복잡한 단계, 굳이 안 써도 됨
-- 부서의 합이 가장 큰 값 // subquery는 어디든 들어갈 수 있음 -> from에 들어갈땐 무조건 별칭이 들어가야함 => 테이블을 만듬
SELECT max(sum_salary)
FROM (SELECT sum(salary) sum_salary
	FROM employee
	GROUP BY dept_code) dept_sum; --  New 테이블 - 최대 큰 값
...
/*
	2. 다중행 서브쿼리
    - 서브쿼리의 조회 결과 값의 개수가 여러 행 일 때 (여러행 연결)
    
    IN 서브쿼리 : 여러개의 결과값 중에서 한개라도 일치하는 값이 있다면
*/
-- 각 부서변 최고 급여를 받는 직원의 이름, 직급코드, 부서코드, 급여 조회
-- 1) 각 부서별 최고 급여
SELECT max(salary)
FROM employee
GROUP BY dept_code;
...
-- 직원에 대한 사번, 이름, 부서코드, 구분(사수/사원) 조회 (manager_id)
select emp_id, emp_name, manager_id from employee;
-- 1) 누군가의 사수인 사람들의 id가 필요
...
SELECT emp_id, emp_name, dept_code, 
	if(emp_id IN (SELECT DISTINCT emp_id
FROM employee
...
/*
	3. 다중열 서브쿼리
    - 서브쿼리 조회 결과값이 한 행이지만 컬럼이 여러개일 때 (한 행 여러 열)
*/
...
SELECT emp_id, emp_name, IFNULL(dept_code,'부서없음') 부서코드, salary
FROM employee
WHERE (IFNULL(dept_code,'부서없음'),salary) IN (SELECT max(salary)
				FROM employee
                GROUP BY IFNULL(dept_code,'부서없음'));
