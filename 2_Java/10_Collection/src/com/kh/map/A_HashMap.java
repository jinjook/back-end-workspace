...
public class A_HashMap {
	 ...
	 * - key-value 쌍으로 저장
	 * - 순서 없음
	 * - 중복 : 키(key)는 중복 X, 값(value)는 상관 없음
	 * 
	 * HashMap
	...
	public void method1() {
		Map<String, Integer> map = new HashMap<>(); // <>에 key, value 값 모두 명시해야함
		map.put("웨하스", 250);
		map.put("고래밥", 180);
		map.put("칸쵸", 800);
		map.put("나쵸", 450);
		// 키나 값이 항상 같은 형태여야하는 단점이 있음
		
		System.out.println(map);
		
		// 1. 키만 가져오기
		Set<String> key = map.keySet(); // 리턴타입 확인 : Set<String>
		System.out.println(key);
		
		// 2. 값만 가져오기
		Collection<Integer> col = map.values(); // 리턴타입 확인 : Collection<Integer>
		System.out.println(col);
		
		// 3. 키에 해당하는 value 값 가져오기 : 키값은 중복이 안되니까
		for(String s : key) {
	...
	public void method2() {
		...
		System.out.println("과자의 개수 : " +map.size()); // 9 : 웨하스 중복
		...
		// 2. 후렌치파이의 맛 정보 출력
		System.out.println("후렌치파이의 맛 : "+map.get("후렌치파이").getFlavor());
		...
		System.out.println("모든 과자의 칼로리 총합 : "+sum);
		System.out.println("모든 과자의 칼로리 평균 : "+ sum/map.size());
