...
public class A_String {
	...
	/* String 클래스
	 * - 불변의 클래스 (객체를 생성하면 변경 불가)
	 * - 변경이 적고 읽기가 많은 경우 사용하는 것이 좋음
	...
	public void method2() {
		...
		//	  substring(int beginIndex, int ednIndex) : String
		// 	  문자열의 beginIndex부터 endIndex-1 까지 (endIndex가 생략되면 끝까지)의 문자열을 새로 생성해서 리턴
		String str3 = str.substring(7); // Java!
		String str4 = str.substring(0, 5); // Hello
		System.out.println("3. substring  : " +str3);
		System.out.println(str4);
		
		// 4. indexOf(String str) : int
		// 	  전달받은 str의 '시작 인덱스'를 리턴 (포함되어 있지 않으면 -1)
		...
		String str5 = str.replace('l', 'c'); // Hecco, Java!
		...
		System.out.println("8. toCharArray : " + arr); // 주소값
	 ...
	 * - 가변의 클래스 : String 클래스와 달리 내부의 문자열을 수정할 수 있음
	 * - 16개의 문자를 저장할 수 잇는 버퍼가 미리 생성되고 문자가 저장됨에 따라 자동 증가
	...
	public void method3() {
		...
		// 1. append(String str) : StringBulider
		//    기존 문자열 뒤에 문자열 추가
		sb.append("Java!");	// Hello, Java!
		System.out.println(sb); // String은 기본 문자 자체 변경 불가, StringBulider는 가능
		...
		// 메서드 체이닝 : 메서드를 이어서 쓸 수 있음
		StringBuilder sb2 = new StringBuilder("Java Program");
		// "IPA avaJ" 만들기
		sb2.delete(5, 12).reverse().insert(0, "IPA"); // 연결가능
	...
	public void method4() {
		...
		System.out.println("과목 개수 : " + strArr.length); // 배열은 length, 리스트는 size
		for(String s : strArr) {
		...
		System.out.println(st);// 주소값으로 출력됨
		...
		*/
		...
		System.out.println("'남아있는' 과목 개수 : " + st.countTokens()); // 12
		
		while(st.hasMoreTokens()) {
			System.out.println(st.nextToken());
		}
		
		System.out.println("'남아있는' 과목 개수 : " + st.countTokens()); // 0
