...
public class A_Match {
	...
	public void method1() {
		...
		System.out.println("짝수만 가지고 있는가(allMatch) : "+result);
		
		result = Arrays.stream(values)
					   .anyMatch(value -> value % 3 == 0);
		System.out.println("3의 배수 값이 하나라도 있는가(anyMatch) : " + result);
		
		result = Arrays.stream(values)
					   .noneMatch(value -> value % 5 ==0);
		System.out.println("모두 다 5의 배수가 아닌가(noneMatch) : " + result);
	...
	public void method2() {
		...
		System.out.println("남학생들 중 평균이 70점 이상인 학생이 한 명이라도 존재하는지 : "+result);
