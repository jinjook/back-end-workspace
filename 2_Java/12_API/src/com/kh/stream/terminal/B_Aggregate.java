...
public class B_Aggregate {
	...
	/* 기본 집계
	 * - 최종 처리 기능으로 요소들을 개수, 합계, 평균값, 최대값, 최소값 등 하나의 값으로 산출하는 역할
	 * 	- count : 요소의 개수 리턴
	 * 	- sum : 요소들의 합계 리턴
	 * 	- average : 요소들의 평균 리턴
	 * 	- max : 최대 요소 리턴
	...
	public void method1() {
		...
		// get : 저장되어 있는 값을 얻기 위해 사용 (getAsDouble : double로 받아내기)
		...
		int maxResult = max.orElse(0);// 0 : 디폴트 값
		...
		// ifPresent : 값이 저장되어있을 경우 처리 (=if + isPresent)
		first.ifPresent(value -> System.out.println("첫번째 값 : "+value));
				
	}
	
	/* 커스텀 집계
	 * - 스트림에서 기본 집계 메서드를 제공하지만, 다양한 집계 결과물을 만들 수 있도록 reduce 메서드를 제공함
	 * */
	public void method2() {
