...
public class B_InDecrease {
	 ...
	 * - ++ : 값 1 증가
	 * - -- : 값 1 감소
	 * - (증감연산자)값; : 앞에 붙었을 때는 전위 연산으로 먼저 증감 연산을 수행하고 다른 연산을 수행
	 * - 값(증감연산자); : 뒤에 붙었을 때는 후위 연산으로 먼저 다른 연산을 수행하고 증감 연산을 수행
	 * 
	 */
...
//		b.method1();
	...
	public void method1() {
		...
		// 후위 연산자
		System.out.println("1회 수행 후 값 : " + number++); // 12 (출력이 되고 난 후 증감)
		System.out.println("2회 수행 후 값 : " + number++); // 13 (이전에 증감 된 값으로 출력이 되고 난 후 증감)
		System.out.println("후위 연산자 적용 후 : " + number); // 14 (이전에 증감 된 값으로 출력)
	}
