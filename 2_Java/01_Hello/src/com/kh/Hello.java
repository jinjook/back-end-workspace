 ...
 *     이름 없는 패키지(default)에 속함
 * */
package com.kh;

/*
 * 클래스(class)
 * - 자바에서 모든 코드는 반드시 클래스 안에 존재해야하며 
 *   서로 관련된 코드들을 그룹으로 나누어 별도의 클래스를 구성
 * - 클래스들이 모여 하나의 Java 프로그램을 구성
 * */
public class Hello { // 클래스명은 클래스 파일명과 동일해야함
	
	/*
	 * 메서드 (method)
	 * - 클래스의 기능(동작)에 해당하는 구현 부분
	 * - 해당 메서드 호출 시 메서드의 블럭({})에 해당하는 부분을 실행
	 * */
	// 실행 메서드 (main method) : 자바 프로그램 실행 시 가장 먼저 호출
	// 모든 클래스가 main method를 가지고 있어야 하는건 아님
	// 하나의 Java 애플리케이션에는 main 매서드를 포함한 클래스가 반드시 하나는 있어야함
	public static void main(String[] args) {
		
		System.out.println("Hello, Java!"); // 실행 : Ctrl + F11
		
	}
	
	/*
	 * 네이밍 규칙
	 * - 패키지 : 소문자로 작성, 회사들 간에 패키지가 서로 중복되지 않도록 보통 회사의 도메인 이름으로 패키지를 만든다.
	 * - 클래스 : 대문자로 시작, 여러 개의 단어가 있을 때는 단어 앞자리마다 대문자로 표기 ex. HelloWorld 
	 * - 메서드 : 소문자로 시작, 여러 개의 단어가 있을 때는 단어 앞자리마다 대문자로 표기 ex. mainWorld
	 * */
}
